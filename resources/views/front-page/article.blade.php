...
@section('content')
    <main class="mx-3 md:max-w-screen-sm pt-5 lg:max-w-screen-md xl:max-w-screen-lg 2xl:max-w-screen-xl md:mx-auto mb-12">
        ...
        <article>
            ...
            <div class="flex items-center justify-center mt-5 gap-2">
                <img class="w-7" src="{{ $article->user->profile->profile_photo_url }}" alt="{{ $article->title }}"
                    id="profile-photo">
                <p class="text-sm 2xl:text-base font-semibold text-sky-500 dark:text-sky-400">{{ $article->user->profile->full_name }}
                    &bull; <span class="text-black dark:text-white font-normal">{{ $article->creation_date }}</span></p>
            </div>
            ...
            <div class="mt-7" id="share-box">
                <p class="text-sm 2xl:text-base text-center">Merasa artikel ini bermanfaat? Jangan lupa bagikan ke temanmu, ya!</p>
                <div class="flex gap-2 mt-3 w-fit mx-auto">
                    ...
                    <a class="py-0.5 px-2 text-white bg-blue-600 rounded-full text-sm 2xl:text-base font-semibold"
                        href="{{ 'https://facebook.com/sharer.php?u=' . url()->full() }}"
                        target="_blank">
